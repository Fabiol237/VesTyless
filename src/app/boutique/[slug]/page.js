...
export default function Storefront({ params }) {
  ...
  const handleShare = async (product = null) => {
    ...
    const shareData = product
      ? { title: product.name, text: `Découvre "${product.name}" à ${Number(product.price).toLocaleString()} FCFA sur ${store?.name} | VesTyle`, url: window.location.href + `?product=${product.id}` }
      : { title: store?.name, text: `Découvre la boutique "${store?.name}" sur VesTyle — l'excellence à proximité !`, url: window.location.href };
    try {
  ...
  if (loading && !store) {
    return (
      <div className="min-h-screen bg-[#F8F9FA] overflow-hidden">
        {/* Skeleton Nav */}
        <nav className="h-24 px-4 md:px-8 flex items-center justify-between">
        ...
        {/* Skeleton Hero */}
        <div className="relative h-[60vh] sm:h-[50vh] min-h-[400px] bg-neutral-200 animate-pulse">
          <div className="absolute bottom-0 inset-x-0 p-6 md:p-12">
            <div className="max-w-6xl mx-auto flex gap-6 md:gap-10 items-end">
              <div className="w-32 h-32 md:w-40 md:h-40 bg-neutral-300 rounded-[2.2rem]"></div>
              <div className="flex-1 pb-4 space-y-4">
                <div className="w-1/3 h-10 bg-neutral-300 rounded-lg"></div>
                <div className="w-1/4 h-4 bg-neutral-300 rounded"></div>
              </div>
            </div>
          </div>
        </div>
        {/* Skeleton Content */}
        <main className="max-w-7xl mx-auto px-4 md:px-8 py-12 flex flex-col lg:flex-row gap-12">
          <aside className="lg:w-64 space-y-4"><div className="h-64 bg-neutral-200 rounded-[2rem] animate-pulse"></div></aside>
          <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-8">
            {[1, 2, 3, 4, 5, 6].map(i => (
              <div key={i} className="bg-white rounded-[2.5rem] border border-neutral-100 overflow-hidden animate-pulse">
    ...
    <div className="min-h-screen bg-[#F8F9FA] text-neutral-900 selection:bg-wa-teal selection:text-white font-sans overflow-x-hidden">
      ...
      <nav className={`fixed inset-x-0 z-[100] transition-all duration-500 px-4 md:px-8 flex items-center justify-between ${isScrolled ? 'h-16 bg-white/90 backdrop-blur-xl shadow-lg border-b border-neutral-100' : 'h-24 bg-transparent'}`}>
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => handleShare()} className={`p-3 rounded-2xl transition-all ${isScrolled ? 'text-neutral-400 hover:text-neutral-900 bg-neutral-50' : 'text-white/80 hover:text-white bg-white/10 backdrop-blur-md border border-white/20'}`} title="Partager la boutique">
            {shareSuccess
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white w-full max-w-md rounded-[3rem] p-8 md:p-12 space-y-8 animate-in fade-in zoom-in duration-300">
        <div className="flex items-center justify-between">
          ...
          <button onClick={onClose} className="p-2 hover:bg-neutral-100 rounded-xl transition-all"><XIcon size={24} /></button>
        </div>
